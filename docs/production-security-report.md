...
- Tightened admin role vocabulary to `owner`, `admin`, `manager`, and `content_manager`.
...
- Some admin CRUD screens still perform Supabase writes from client components. RLS now provides the real authorization boundary, but the enterprise target should move these writes behind `/app/api/admin/*` controllers.
...
- Apply all migrations in `supabase/migrations`, including `20260612002000_enterprise_security_hardening.sql`.
